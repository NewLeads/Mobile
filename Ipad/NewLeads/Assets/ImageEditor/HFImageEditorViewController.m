...
{
	self.frameView.frame = CGRectMake(self.frameView.frame.origin.x,
									  self.frameView.frame.origin.y,
									  self.view.bounds.size.width, self.view.bounds.size.height - self.frameView.frame.origin.y);
...
- (void)updateCropRect
{
//	self.cropRect = CGRectMake(roundf((self.frameView.bounds.size.width-self.cropSize.width)/2),
//							   roundf((self.frameView.bounds.size.height-self.cropSize.height)/2),
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        CGImageRef resultRef = [self newTransformedImage:self.imageView.transform
                                  ...
                                  sourceOrientation:self.sourceImage.imageOrientation
                                        outputWidth:self.outputWidth ? (self.outputWidth * [UIScreen mainScreen].scale) : self.sourceImage.size.width
...
//    uiCoords = CGAffineTransformTranslate(uiCoords, cropSize.width/2.0, cropSize.height/2.0);
	...
	uiCoords = CGAffineTransformTranslate(uiCoords,
//										  (cropRect.size.width + cropRect.origin.x)/2.0,
//										  (cropRect.size.height + cropRect.origin.y)/2.0);
