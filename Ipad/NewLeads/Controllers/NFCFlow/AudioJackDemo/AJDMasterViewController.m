...
@property (weak, nonatomic) id nfcDelegate;
...
-(void)setMasterDelegate:(id<AJDMasterViewControllerDelegate>)masterDelegate andReader:(ACRAudioJackReader*)anReader
{
    self.nfcDelegate = masterDelegate;
    _reader = anReader;
}
...
- (void)viewDidLoad
{
    ...
    _trackDataOption = ACRTrackDataOptionEncryptedTrack1 | ACRTrackDataOptionEncryptedTrack2 | ACRTrackDataOptionMaskedTrack1 | ACRTrackDataOptionMaskedTrack2;
    ...
    [_reader setTrackDataOption:_trackDataOption];
...
- (IBAction)onDone:(id)sender
{
    if (self.nfcDelegate)
    {
            [_reader setDelegate:nil];
            [_reader piccPowerOff];
            _reader = nil;
            AudioSessionRemovePropertyListenerWithUserData(kAudioSessionProperty_AudioRouteChange, AJDAudioRouteChangeListener, (__bridge void *)(self));
        
            [self.nfcDelegate didFinishedAJDMasterViewController];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([[segue identifier] isEqualToString:@"AboutReader"]) {
        ...
        [trackDataViewController setTrackDataOptions:_trackDataOption];
...
cleanup:
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.nfcDelegate)
        {
            ...
            if(track1Data.track1String.length>0)
                [encodedData appendFormat:@"Track 1:%@Z", track1Data.track1String];
            if(track2Data.track2String.length>0)
                [encodedData appendFormat:@"Track 2:%@Z", track2Data.track2String];

            [self.nfcDelegate didReceivedEncodedData:encodedData];
        }
...
- (void)reader:(ACRAudioJackReader *)reader didSendRawData:(const uint8_t *)rawData length:(NSUInteger)length {
    ...
    NSLog(@"rawData = %@", hexString);
    
    BOOL isCheckSumOK = [_reader verifyData:rawData length:length];

    NSString* chexString = [hexString stringByAppendingString:isCheckSumOK ? @" (Checksum OK)" : @" (Checksum Error)"];

    dispatch_async(dispatch_get_main_queue(), ^{

        self.dataReceivedLabel.text = chexString;
        [self.tableView reloadData];
        
        if (isCheckSumOK)
        {
            if (length>7 && self.nfcDelegate)
            {
                [self.nfcDelegate didReceivedRawData:hexString];
            }
        }
